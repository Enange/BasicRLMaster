...
class Network_cont(nn.Module):
    ...
    def forward(self, x):
        ...
        output = (res * T.from_numpy((self.output_range[1] - self.output_range[0])) + T.from_numpy(
            self.output_range[0]))  # in range [0,1]
...
class DDPG_PT:
    ...
    def loop(self, num_episodes=1000):
        ...
        for episode in range(num_episodes):
            ...
            state, info = self.env.reset(seed=val, options={})
            ep_reward = 0
            ...
            if self.verbose > 1:
                np.savetxt(f"data/PyGame/DDPG{self.seed}_reward.txt", reward_list)
                np.savetxt(f"data/PyGame/DDPG{self.seed}_success.txt", success_list)
                np.savetxt(f"data/PyGame/DDPG{self.seed}_collision.txt", collision_list)
